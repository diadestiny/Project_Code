...
class YoloDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.mosaic and self.rand() < self.mosaic_prob and self.epoch_now < self.epoch_length * self.special_aug_ratio:
            ...
            side_image, _ = self.get_random_data(self.annotation_lines[index].replace('cam1','cam2'), self.input_shape, random=self.train)

        image       = np.transpose(preprocess_input(np.array(image, dtype=np.float32)), (2, 0, 1))
        side_image = np.transpose(preprocess_input(np.array(side_image, dtype=np.float32)), (2, 0, 1))
        ...
        return image, box, y_true, side_image
...
def yolo_dataset_collate(batch):
    ...
    side_images = []
    for img, box, y_true,side_img in batch:
        images.append(img)
        bboxes.append(box)
    ...
    side_images = torch.from_numpy(np.array(side_images)).type(torch.FloatTensor)
    return images, bboxes,y_trues,side_images
