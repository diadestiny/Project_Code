...
out_file = './static/test_show/'
dir_path = './static/test_detection/'
heat_path = os.path.join(dir_path,"heatmap")
# coco = COCO(json_file)
config_file = './model_data/detectors_cascade_rcnn_r50_1x_coco.py'
checkpoint_file = './model/detection.pth'
...
# inference_detector(model,"./static/upload/detection/000489.bmp")
...
def deteciton_demo(img_path):
    ...
    with torch.no_grad():
        ...
        if not os.path.exists(os.path.join("./static/detection_ShipRSImageNet_V1_dataset_xml/"+image_name_id+".xml")):
        ...
        with open(os.path.join(dir_path, "ground-truth/"+image_name_id+".txt"), "w") as new_f:
            root = ET.parse(os.path.join("./static/detection_ShipRSImageNet_V1_dataset_xml/"+image_name_id+".xml")).getroot()
        ...
        new_f.close()
        # map,ap_dictionary = get_map(0.5,False, image_name_id,score_threhold = 0.5,path='./static/test_detection')
        ap_dictionary = {}
