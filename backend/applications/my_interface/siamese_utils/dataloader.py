...
class SiameseDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        raw_image = cvtColor(raw_image)
        if self.autoaugment_flag:
            image = self.AutoAugment(image, random=self.random)
            raw_image = self.AutoAugment(raw_image, random=self.random)
        else:
            image = self.get_random_data(image, self.input_shape, random=self.random)
            raw_image = self.get_random_data(raw_image, self.input_shape, random=self.random)
    ...
    def _convert_path_list_to_images_and_labels(self, path_list,class_list:list):
        ...
        for pair in range(number_of_pairs):
            ...
            if class_list[pair] == 1:
                labels[pair] = 1
            elif class_list[pair] == 2:
                labels[pair] = 2
            elif class_list[pair] == 3:
                labels[pair] = 3
            else:
                labels[pair] = 0
            # if (pair + 1) % 2 == 0:
...
def dataset_collate(batch):
    ...
    images = []
    targets = []
    for raw_image, image, y in batch:
        raw_images.append(raw_image)
        images.append(image)
        targets.append(y)
    raw_images = torch.from_numpy(np.array(raw_images)).type(torch.FloatTensor)
    images = torch.from_numpy(np.array(images)).type(torch.FloatTensor)
    targets = torch.from_numpy(np.array(targets)).type(torch.FloatTensor).long()
    return raw_images, images, targets
