...
def fit_one_epoch(model_train, model, loss, loss_history, optimizer, epoch, epoch_step, epoch_step_val, gen, genval, Epoch, cuda, fp16, scaler, save_period, save_dir, local_rank=0):
    ...
    for iteration, batch in enumerate(gen):
        if iteration >= epoch_step:
            break
        raw_images, images, targets = batch[0], batch[1], batch[2]
        with torch.no_grad():
            if cuda:
                raw_images = raw_images.cuda(local_rank)
        ...
        optimizer.zero_grad()
        if not fp16:
            outputs = model_train(raw_images,images)
    ...
    if local_rank == 0:
        ...
        print('val accuracy:',val_total_accuracy /epoch_step_val)
